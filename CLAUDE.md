...
- **Unit tests**: Only generated stubs exist (`ExampleUnitTest` / `ExampleInstrumentedTest`); no real test coverage. Stubs live under `com.blankm.*` while production code is `me.blankm.*`.
- **Device testing**: Core flows (camera capture, album pick, manual crop, front/back sequencing) verified on Android 14 physical device. Lower Android versions and edge cases (low memory, specific ROM variants, accessibility services) untested in this session but covered by lint fixes and defensive coding.
...
- **Album pick** — the picked `Uri` is resolved through `UriUtils` (branching on Android Q for `uriToFileApiQ` vs `getFileFromMediaUri` + EXIF rotation) into `AlbumClipImageView` for pan/zoom, then `createClippedBitmap()` produces the final crop.

  Note the `BitmapRegionDecoder` branch in `createClippedBitmap()` is **dead code (1)**: `mSampleSize`, `mDegree`, `mSourceWidth`, `mSourceHeight`, and `mMaxWidth` are declared but never assigned anywhere in the class, so `mSampleSize` is always 0 and the method always returns early via `mAlbumClipIv.clip()`. `findBestSample()` and `getRealRect()` are likewise unreachable. Either wire those fields up when the source image is loaded, or delete the branch — don't assume it runs.

  **Dead code (2):** `AutoFocusManager` (timer-based autofocus) is never constructed — the only `new AutoFocusManager(...)` is commented out at `CameraPreview.java:102`, so `mAutoFocusManager` is permanently null and its `stop()`/null-out block in `release()` never fires. Focus actually comes from `SensorControler` (accelerometer-driven) plus the touch-to-focus `focus()` call. `AutoFocusManager` is the only remaining `AsyncTask` user in the codebase; it was left as-is rather than modernized because there is no way to exercise it. Revive it or delete it — don't modify it expecting a behavior change.
...
- **Logging goes through `utils/LogUtils`, never `android.util.Log` or `System.out` directly.** `d`/`w` are gated on `BuildConfig.DEBUG` so nothing prints from a release aar; `e` always prints so real failures stay diagnosable on production. Consumers can force logging on with `LogUtils.setDebug(true)`. Swallowing an exception with `printStackTrace()` is not acceptable — route it to `LogUtils.e`/`w` with a message describing the specific operation that failed.
- **The library adds no third-party dependencies.** CameraX and appcompat are deliberately `compileOnly` so consumers control their own versions. That rules out Timber, Hilt/Dagger, ViewModel/LiveData, and Glide inside `idcard_lib` regardless of what general Android guidance suggests — use plain `ExecutorService` + a main-thread `Handler` for async work.
- **No image decoding, cropping, or file writes on the main thread.** Both camera activities own an `mIoExecutor` (single thread) plus `mMainHandler`, shut down in `onDestroy` alongside `removeCallbacksAndMessages(null)`. Read view geometry on the main thread, hand the values to the background task, and re-check `isFinishing() || isDestroyed()` before touching UI in the posted result. Decode with `ImageUtils.decodeSampledBitmap` rather than bare `BitmapFactory.decodeFile`.
- **Results must be returned with `putStringArrayListExtra`**, matching the `getStringArrayListExtra` in `IDCardCameraSelect.getImagePath`. `CameraActivity` funnels this through `setResultAndFinish()`; keep new exit paths going through it. Only add a result path once the file is confirmed written.
