...
public class AlbumClipImageView extends AppCompatImageView implements ScaleGestureDetector.OnScaleGestureListener, View.OnTouchListener {
    ...
    public AlbumClipImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
                ...
                new SimpleOnGestureListener() {
                    ...
                    public boolean onDoubleTap(MotionEvent e) {
                        ...
                        if (getScale() < mScaleMin) {
                            AlbumClipImageView.this.postDelayed(new AutoScaleRunnable(mScaleMin, x, y), 16);
                        } else {
                            AlbumClipImageView.this.postDelayed(new AutoScaleRunnable(mInitScale, x, y), 16);
                        }
                        isAutoScale = true;
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AlbumClipImageView);
        mAspectX = ta.getInteger(R.styleable.AlbumClipImageView_civWidth, 1);
        mAspectY = ta.getInteger(R.styleable.AlbumClipImageView_civHeight, 1);
        mClipPadding = (int) ta.getDimension(R.styleable.AlbumClipImageView_civClipPadding, 0);
        mTipText = ta.getString(R.styleable.AlbumClipImageView_civTipText);
        mMaskColor = ta.getColor(R.styleable.AlbumClipImageView_civMaskColor, 0xB2000000);
        mDrawCircleFlag = ta.getBoolean(R.styleable.AlbumClipImageView_civClipCircle, false);
        mRoundCorner = ta.getDimension(R.styleable.AlbumClipImageView_civClipRoundCorner, 0);
        final int textSize = ta.getDimensionPixelSize(R.styleable.AlbumClipImageView_civTipTextSize, 24);
    ...
    private class AutoScaleRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (((tmpScale > 1f) && (currentScale < mTargetScale))
                ...
                AlbumClipImageView.this.postDelayed(this, 16);
            } else {
                // 设置为目标的缩放比例
